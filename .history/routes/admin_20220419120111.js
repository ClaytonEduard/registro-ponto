...
router.get("/pontos", (req, res) => {
    ...
    Ponto.find().then((pontos) => {
        ...
        for (let i = 0; i < pegar.length; i++) {
            ...
            if (pegar[i].tipo == '0') {
                //console.log(pegar[i])
                ver.push(pegar[i].datalancamento.toString('DD/MM/AAAA - 00:00:00.000'))
            } else {
                out.push(pegar[i].datalancamento.toString('DD/MM/AAAA - 00:00:00.000'))

            }

        }
        console.table("VER: " + ver)
