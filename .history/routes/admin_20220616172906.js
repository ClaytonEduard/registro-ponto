...
router.get("/pontos", (req, res) => {
    ...
    Ponto.find().then((pontos) => {
        ...
        function obterPonto(pontos, id, tipo, matricula) {
            const ponto = pontos.find((ponto) => (ponto === null || ponto === void 0 ? void 0 : ponto.id) === id && tipo === (ponto == null || ponto === void 0 ? void 0 : ponto.tipo) && (matricula === null || matricula === void 0 ? void 0 : matricula.matricula))
             console.log(ponto)
            return ponto
        }
        ...
        function obterRelatorio(pontos) {
            ...
            while (idsUnico[indice]) {
                ...
                while (registrosDoUsuario[indiceDois]) {
                    ...
                    indiceDois = indiceDois+1
                    const matriculaProxid = registrosDoUsuario[indiceDois].matricula;                    //  console.log(proximoId)
                    const tipoEntrada = '0';
                    const tipoSaida = '1';
                    const par = 2;
                    const entrada = obterPonto(pontos, idAtual, tipoEntrada, matriculaAtual);
                    console.log(entrada);
                    const saida = obterPonto(pontos, proximoId, tipoSaida, matriculaProxid);
                    console.log(saida)
                    if (saida == undefined) indiceDois + 1
                    console.log(saida)
                    // if (entrada.matricula == saida.matricula) {
