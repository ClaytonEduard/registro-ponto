...
router.get("/pontos", (req, res) => {
    ...
    const entrada = await Ponto.find({ tipo: '0' }).then((pontos) => {
        //res.render('admin/pontos', {
        pontos: pontos
    })

    const entrada = await Ponto.find({ tipo: '1' }).then((pontos) => {
        res.render('admin/pontos', {
            pontos1: pontos1
        })
    }).catch((err) => {
        req.flash("error_msg", "Houve um erro ao listar os pontos!");
        res.redirect("/admin");
    })
})
